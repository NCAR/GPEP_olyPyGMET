clc;clear;close all
Outfigure='trange_metric_map_nrmse';
...
% data preparation
datafile='trange_plotdata_nrmse.mat';
if exist(datafile,'file')
    load(datafile,'latg','long','metmerge_grid','metcorr_grid','metdown_grid','metraw_grid');
else
    load('trange_evaluation.mat','met_merge','met_corr','met_raw','met_down','lle');
    load('../../DEM/NA_DEM_010deg_trim.mat','DEM','InfoLow');
    
    load('mean_value.mat','mean_stn');
    met_merge(:,2)=met_merge(:,2)./mean_stn(:,3);
    for i=1:3
        met_corr(i,:,2)=squeeze(met_corr(i,:,2))'./mean_stn(:,3);
        met_raw(i,:,2)=squeeze(met_raw(i,:,2))'./mean_stn(:,3);
        met_down(i,:,2)=squeeze(met_down(i,:,2))'./mean_stn(:,3);
    end
