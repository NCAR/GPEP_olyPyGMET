...
% data preparation
datafile='tmean_plotdata.mat';
if exist(datafile,'file')
    load(datafile,'latg','long','metmerge_grid','metcorr_grid','metdown_grid','metraw_grid');
else
    load('tmean_evaluation.mat','met_merge','met_corr','met_raw','met_down','lle');
...
for i=1:3
    ...
    dataplot{3,i}=squeeze(metraw_grid(i,:,:,2)); %rmse
    dataplot{2,i}=squeeze(metmerge_grid(:,:,1)); % merge
    dataplot{4,i}=squeeze(metmerge_grid(:,:,2)); % merge
end

% plot
% basic settings
clims=cell(4,3);
clims(1:2,1:3)={[0.94,1]};
clims(3:4,1:3)={[0,5]};
cbins=[6,6,6,6];

fsize=4;
xtitles1={'ERA5','MERRA-2','JRA-55'};
xtitles2={'BMA','BMA','BMA'};
ytitles1={'CC','CC','RMSE','RMSE'};
...
%         end
        if j==1
            yh=ylabel(ytitles1{i},'fontsize',fsize+4,'fontweight','bold');
            yh.Position=yh.Position+[-0.2 0 0];
        end
        if mod(i,2)==1
            text(-0.7,1.4,xtitles1{j},'fontsize',fsize+3);
        else
            text(-0.7,1.4,xtitles2{j},'fontsize',fsize+3);
        end
